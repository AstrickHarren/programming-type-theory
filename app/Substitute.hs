...
-- M[z/x][N/y]
subsub :: Substitute a => Var -> Var -> Var -> Term -> a -> a
subsub x z y n m = sub y n (sub x (Term z) m)

instance Substitute Term where
  -- x[N/x] = N; x[N/y] = x;
  sub y n t@(Term x) = if x == y then n else t
  -- (λx:τ.M)[N/x] = (λx:τ.M);
  -- (λx:τ.M)[N/y] = (λz:τ[z/x][N/y].M[z/x][N/y]) w/ z ∉ FV(N) ∪ FV(M) ∪ FV(τ) if x ∈ FV(N)
  -- (λx:τ.M)[N/y] = (λx:τ[N/y].M[N/y]) if x ∉ FV(N)
  sub y n t@(Lambda (TypedVar x τ) m)
    | x == y = t
    | x `member` fv n =
      let z = varNotIn (fv t `union` fv n)
       in Lambda (z `TypedVar` subsub x z y n τ) (subsub x z y n m)
...
instance Substitute Type where
  ...
  -- (∀x:ϕ.ψ)[N/x] = (∀x:ϕ.ψ)[N/x]
  -- (∀x:ϕ.ψ)[N/y] = (∀z:ϕ[z/x][N/y].ψ[z/x][N/y]) w/ z ∉ FV(ϕ) ∪ FV(ψ) ∪ FV(N) if x ∈ FV(N)
  -- (∀x:ϕ.ψ)[N/y] = (∀x:ϕ[N/y].ψ[N/y]) o.w.
  sub y n t@(Forall (TypedVar x ϕ) ψ)
    | x == y = t
    | x `member` fv n =
      let z = varNotIn (fv t `union` fv n)
       in Forall (z `TypedVar` subsub x z y n ϕ) (subsub x z y n ψ)
